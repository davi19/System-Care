...
namespace SystemCare
{
    ...
    {
        QuerryMysql VizualizaHistorico = new QuerryMysql();
        public Historico(string IdFuncionario)
        {
            InitializeComponent();
            DataTable TabelaHiostorico = VizualizaHistorico.SelecionaHistoricoFuncionario(IdFuncionario);
        ...
        private void GridHistorico_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                ...
                var TabelaDadosFuncionario = VizualizaHistorico.RecuperaDadosFuncionario(TabelaAso.Rows[0]["idfuncionario"].ToString());
                ...
                string[] ModalidadeExames = TabelaAso.Rows[0]["modalidadeexame"].ToString().Split(';');
                ...
                for (var i = 1; i < ModalidadeExames.Length; i++)
                {
                    try
                    {
                        ...
                        ModalidadeExameAos = ModalidadeExameAos + VizualizaHistorico.RetornaModalidadeExame(ModalidadeExames[i])+ "\n";
                    }
                    catch
                    {
                    }
                ...
                var AOS = new Relatorios(TabelaDadosFuncionario.Rows[0]["nome"].ToString(),
                    ...
                    TipoExameAos, Sexo,
                    Convert.ToDateTime(TabelaDadosFuncionario.Rows[0]["datanascimento"].ToString()).Date.ToShortDateString(),
